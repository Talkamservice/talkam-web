...
export const GroupMedia = () => {
    ...
    const { data: groupMedia, isLoading, isError, error } = useGetAllPostsQuery({
        // tab: 'groupMedia',
        type: "Media",
        page: page,
        groupId: groupId
    ...
    const appendNewPageData = () => {
        if (groupMedia?.data?.data) {
            setPosts((prevPosts) => {
                const newPosts = new Set([...prevPosts, ...groupMedia.data.data]);
    ...
    const handleScroll = useCallback((event) => {
        ...
        if (bottom && !isFetching && groupMedia?.data?.pagination_meta.can_load_more) {
            setIsFetching(true);
            setPage((prevPage) => prevPage + 1);
        }
        Storage.setItem("scrollPosition_groupgroupMedia", scrollTop);
    }, [isFetching, groupMedia]);

    const restoreScrollPosition = () => {
        const savedScrollPosition = Storage.getItem("scrollPosition_groupgroupMedia");
            ...
            >
                {
                    isLoading ?
                        ...
                        newResults?.length > 0 ?
                            ...
                            :
                            !isLoading && !isFetching && groupMedia.data.data.length === 0 ?
