...
export const GroupPosts = () => {
    ...
    const { data: groupPosts, isLoading, isError, error } = useGetAllPostsQuery({
        // tab: 'groupPosts',
        page: page,
        groupId: groupId
    ...
    const appendNewPageData = () => {
        if (groupPosts?.data?.data) {
            setPosts((prevPosts) => {
                const newPosts = new Set([...prevPosts, ...groupPosts.data.data]);
    ...
    const handleScroll = useCallback((event) => {
        ...
        if (bottom && !isFetching && groupPosts?.data?.pagination_meta.can_load_more) {
            setIsFetching(true);
            setPage((prevPage) => prevPage + 1);
        }
        Storage.setItem("scrollPosition_groupgroupPosts", scrollTop);
    }, [isFetching, groupPosts]);

    const restoreScrollPosition = () => {
        const savedScrollPosition = Storage.getItem("scrollPosition_groupgroupPosts");
            ...
            >
                {
                    isLoading ?
                        ...
                        newResults?.length > 0 ?
                            ...
                            :
                            !isLoading && !isFetching && groupPosts.data.data.length === 0 ?
