...
export const DropDownSelect = ({defaultValue, options, label, onChange, readOnly, node, styles, buttonStyles}) => {
  ...
  return (
    <div tabIndex={0} className={`w-max space-y-1 ${styles}`}>
      {!!label ? (
      ...
      <button type='button' ref={ref} onClick={toggleDropDown} className={`w-full relative flex justify-between items-center focus:ring-tblue-100 focus:border-tprimary-100
        gap-4 bg-twhite-100 border border-tgray-50 focus:outline-none rounded-md focus:ring-4 cursor-pointer p-3 ${buttonStyles}`}
      >
        {!!node && node}
        ...
        <div className={`absolute overflow-auto ${show ? "block" : "hidden"} left-0 top-full min-w-full w-max max-h-[200px] no-scrollbar bg-white shadow-md mt-1 rounded-lg z-20`}>
          <ul className='overflow-hidden text-left border border-tgray-50 rounded-lg no-scrollbar'>
            ...
            <li className='overflow-hidden z-10 bg-twhite-100 px-4 py-3 hover:bg-tprimary-50 hover:text-white font-medium text-tblack-100 text-xs'>No options available</li>
            }
          </ul>
        </div>
      </button>
