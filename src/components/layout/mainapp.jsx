...
export const MainAppLayout = ({ children }) => {
  ...
  const { data: followingCategories, isLoading: followingCategoriesLoading } =
    useFollowingCategoriesQuery({ skip: !token });
  ...
  const { data: notificationStats, refetch: refetchNotification } =
    useGetNotificationStatsQuery(null, {
      skip: !token,
    });
  ...
  return (
    <section className="w-full flex items-center justify-center no-scrollbar">
      <LoadingBar
        height={3}
        color="#017FC8"
        progress={state === "loading" ? 75 : 100}
      />
      <main className="w-full relative h-dvh no-scrollbar no-scrollbar">
        {/* Mobile header */}
        <header className="w-full border-b border-tgray-light">
          <section
            className={`sticky w-full max-w-screen-2xl mx-auto flex items-center justify-between gap-8 sm:gap-4 bg-white z-40 px-6 lg:px-24 h-[7dvh] py-2 top-0`}
          >
            <div
              onClick={() => {
                navigate("/home");
                ...
                {!isAuth ? (
                  <section className="flex items-center gap-4">
                    ...
                    <Button
                      ...
                      leftIcon={
                        <LockIcon className="w-5 h-5 text-twhite-100" />
                      }
                  ...
                  <>
                    ...
                    <div ref={popUpRef} className="cursor-pointer relative">
                      <Avatar
                        ...
                        src={user?.data?.avatar ?? currentUser?.avatar}
                        size={isMobile ? "xs" : "sm"}
                      />
                      {profileMenu ? (
                        <motion.div
        ...
        <div className="relative flex mx-auto no-scrollbar h-[calc(100dvh-7dvh)] max-w-screen-2xl">
          <div
          ...
          <aside
            className={`fixed border-r border-tgray-light inset-0 z-[38] lg:absolute w-80 sm:w-96 no-scrollbar overflow-y-auto bg-white sm:pl-20 pr-6
            ...
            <div className="flex flex-col h-full overflow-y-auto no-scrollbar">
              <nav className={`flex-1 no-scrollbar ${isMobile && "pt-14"} `}>
                ...
                <section className="flex flex-col items-start gap-3 border-b border-tgray-200 py-6 pl-4 pb-4">
                  ...
                  <Link
                    className="flex items-center justify-between !text-sm !py-0 !px-0"
                    to="/help&info/child-safety-policy"
                  >
                    Child Safety Policy
                  </Link>
                  <Link
